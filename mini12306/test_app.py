...
class TestMini12306Core(unittest.TestCase):
    ...
    def setUp(self):
        ...
        c.execute("UPDATE orders SET pay_status='已取消', status='已退票' WHERE username=? AND create_time LIKE ?",
    ...
    def _get_available_train(self, days_ahead=5):
        ...
        c.execute("SELECT train_no, type, price, start, end, remaining_tickets, total_tickets FROM trains WHERE travel_date=? AND remaining_tickets > 0 LIMIT 1", (future_date,))
        row = c.fetchone()
        conn.close()
        if not row:
            raise unittest.SkipTest("无可测试车次")
        return dict(row)

    def _get_future_train_same_route(self, start, end, exclude=None):
        """找一个未来未发车的同路线车次"""
        conn = get_db()
        c = conn.cursor()
        today = datetime.now().strftime("%Y-%m-%d %H:%M")
        if exclude:
            c.execute("SELECT * FROM trains WHERE start=? AND end=? AND (travel_date > date('now') OR (travel_date = date('now') AND depart_time > time('now'))) AND remaining_tickets > 0 AND train_no != ? ORDER BY travel_date, depart_time LIMIT 1",
                      (start, end, exclude))
        else:
            c.execute("SELECT * FROM trains WHERE start=? AND end=? AND (travel_date > date('now') OR (travel_date = date('now') AND depart_time > time('now'))) AND remaining_tickets > 0 ORDER BY travel_date, depart_time LIMIT 1",
                      (start, end))
        row = c.fetchone()
        conn.close()
    ...
    def _buy_ticket(self, train_no, seat="二等座", passenger="测试乘客", is_student="n"):
        resp = self.client.post("/buy", data={
            "username": self.username, "train_no": train_no,
            "seat": seat, "passenger_name": passenger, "is_student": is_student,
        })
    ...
    def _bulk_buy(self, seat="二等座", passenger="张三"):
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute("SELECT * FROM orders WHERE order_no=?", (buy_res["order_no"],))
    ...
    def test_buy_daily_limit(self):
        ...
        conn = sqlite3.connect("data.db"); c = conn.cursor()
        c.execute("UPDATE sys_config SET value='20' WHERE key='daily_buy_limit'")
        c.execute("UPDATE orders SET status='已退票', pay_status='已取消' WHERE username=? AND create_time LIKE ?",
    ...
    def test_buy_student_discount(self):
        ...
        seat_json = c.fetchone()["seat_types"]; conn.close()
    ...
    def test_refund_success(self):
        ...
        self.assertIn("退票成功", resp.get_data(as_text=True))
        conn = get_db(); c = conn.cursor()
    ...
    def test_refund_unpaid(self):
        ...
        order_id = c.fetchone()["id"]; conn.close()
    ...
    def test_refund_departed(self):
        conn = get_db(); c = conn.cursor()
        yesterday = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
        c.execute("SELECT o.id FROM orders o JOIN trains t ON o.train_no = t.train_no WHERE o.pay_status='已支付' AND o.status='已购票' AND t.travel_date <= ? LIMIT 1", (yesterday,))
        row = c.fetchone()
        conn.close()
        if row:
    ...
    def test_reschedule_cheaper(self):
        ...
        o = c.fetchone()
        c.execute("SELECT * FROM trains WHERE start=? AND end=? AND (travel_date > date('now') OR (travel_date = date('now') AND depart_time > time('now'))) AND remaining_tickets > 0 AND price < ? ORDER BY travel_date, depart_time LIMIT 1",
                  (o["start"], o["end"], o["price"]))
        cheaper = c.fetchone()
        conn.close()
        if not cheaper:
            self.skipTest("没有更便宜的改签目标")
        resp = self.client.post("/reschedule", data={"order_id": order["id"], "new_train_no": cheaper["train_no"], "new_seat_type": order["seat_type"]})
        data = resp.get_json()
        self.assertTrue(data["success"], f"改签失败: {data.get('msg','')}")
    ...
    def test_reschedule_order_not_found(self):
        resp = self.client.post("/reschedule", data={"order_id": 99999999, "new_train_no": "G1_2099-12-31", "new_seat_type": "二等座"})
        data = resp.get_json()
        self.assertFalse(data["success"])
    ...
    def test_reschedule_unpaid(self):
        ...
        c.execute("SELECT id FROM orders WHERE order_no=?", (buy_res["order_no"],))
        order_id = c.fetchone()["id"]; conn.close()
        resp = self.client.post("/reschedule", data={"order_id": order_id, "new_train_no": train["train_no"], "new_seat_type": "二等座"})
        data = resp.get_json()
        self.assertFalse(data["success"])
    ...
    def test_reschedule_already_done(self):
        ...
        c.execute("SELECT * FROM orders WHERE id=?", (order["id"],))
        o = c.fetchone()
        new_train = self._get_future_train_same_route(o["start"], o["end"], o["train_no"])
        conn.close()
        if not new_train:
            self.skipTest("没有改签目标")
        resp1 = self.client.post("/reschedule", data={"order_id": order["id"], "new_train_no": new_train["train_no"], "new_seat_type": order["seat_type"]})
        self.assertTrue(resp1.get_json()["success"])
        resp2 = self.client.post("/reschedule", data={"order_id": order["id"], "new_train_no": new_train["train_no"], "new_seat_type": order["seat_type"]})
    ...
    def test_reschedule_train_not_found(self):
        order = self._bulk_buy()
        resp = self.client.post("/reschedule", data={"order_id": order["id"], "new_train_no": "G99999_2099-12-31", "new_seat_type": "二等座"})
        data = resp.get_json()
        self.assertFalse(data["success"])
    ...
    def test_reschedule_no_tickets(self):
        ...
        c.execute("SELECT * FROM orders WHERE id=?", (order["id"],))
        o = c.fetchone()
        target = self._get_future_train_same_route(o["start"], o["end"], o["train_no"])
        if not target:
            conn.close()
            self.skipTest("没有改签目标")
        c.execute("UPDATE trains SET remaining_tickets=0 WHERE train_no=?", (target["train_no"],))
        conn.commit(); conn.close()
        resp = self.client.post("/reschedule", data={"order_id": order["id"], "new_train_no": target["train_no"], "new_seat_type": "二等座"})
        data = resp.get_json()
        self.assertFalse(data["success"])
    ...
    def test_reschedule_different_city(self):
        ...
        c.execute("SELECT * FROM orders WHERE id=?", (order["id"],))
        o = c.fetchone()
        c.execute("SELECT * FROM trains WHERE start NOT IN (?,?) AND end NOT IN (?,?) AND (travel_date > date('now') OR (travel_date = date('now') AND depart_time > time('now'))) AND remaining_tickets > 0 LIMIT 1",
                  (o["start"], o["start"], o["end"], o["end"]))
        diff = c.fetchone()
        conn.close()
        if not diff:
            self.skipTest("没有不同城车次")
        resp = self.client.post("/reschedule", data={"order_id": order["id"], "new_train_no": diff["train_no"], "new_seat_type": "二等座"})
        data = resp.get_json()
        self.assertFalse(data["success"])
    ...
    def test_reschedule_plane_to_plane(self):
        ...
        c.execute("SELECT * FROM trains WHERE type='飞机' AND travel_date=? AND remaining_tickets > 0 LIMIT 1", (future_date,))
        plane = c.fetchone()
        conn.close()
        if not plane:
        ...
        c.execute("SELECT * FROM trains WHERE type='高铁' AND (travel_date > date('now') OR (travel_date = date('now') AND depart_time > time('now'))) AND remaining_tickets > 0 LIMIT 1")
        train = c.fetchone()
        conn.close()
        if not train:
    ...
    def test_reschedule_cannot_again(self):
        ...
        if not target:
            self.skipTest("没有改签目标")
        resp1 = self.client.post("/reschedule", data={"order_id": order["id"], "new_train_no": target["train_no"], "new_seat_type": order["seat_type"]})
        ...
        conn.close()
        if new_order_row:
            resp2 = self.client.post("/reschedule", data={"order_id": new_order_row["id"], "new_train_no": target["train_no"], "new_seat_type": order["seat_type"]})
            data2 = resp2.get_json()
            self.assertFalse(data2["success"])
            self.assertIn("不可改签", data2["msg"])
